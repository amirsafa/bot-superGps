do
local Amir = 253756858 --put your id here(BOT OWNER ID)
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = '----'
    ...
    if not value then
	 if result.peer_id == tonumber(Amir) then
	   text = text..'مقام : ادمین کل \n\n'
	   elseif result.peer_id == tonumber(Sosha) then
	   text = text..'Rank : مدیر ارشد ربات ⭐️⭐️⭐️ (Full Access Admin) \n\n'
...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
  ...
  if not value then
	 if result.peer_id == tonumber(Amir) then
	   text = text..'مقام : ادمین و سازنده ربات ⭐️⭐️⭐️⭐️⭐️ \n\n'
	   elseif result.peer_id == tonumber(Sosha) then
	   text = text..'مدیر ارشد ربات ⭐️⭐️⭐️شد ربات (Full Access Admin) \n\n'
	   elseif result.peer_id == tonumber(Sosha2) then
	   text = text..'مدیر ارشد ربات ⭐️⭐️⭐️شد ربات (Full Access Admin) \n\n'
	  elseif is_admin2(result.peer_id) then
	   text = text..'مقام : ادمین \n\n'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' or "من کیم" and not matches[2] then
  local receiver = get_receiver(msg)
  local Reply = msg.reply_id
  if msg.reply_id then
...
--------------------
  ...
  if matches[1]:lower() == 'info' or "این کیه" and matches[2] then
